...
#define MIN_TIMER_DURATION 100 //100miliseconds
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	timeval timeout;
	timeout.tv_sec = 0;
	timeout.tv_usec = wait_timeout * 1000; //10millisecond
	...
	while(running)
	{
		...
		for(u_int = 0; i < excep_set.fd_count; i++)		
		{
			SOCKET fd = excep_set.fd_arrray[i];
			CBaseSocket* pSocket = FindBaseSocket((net_handle_t)fd);
			if(pSocket)
			{
				pSocket->OnClose();
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	if((socket_event & SOCKET_READ) !=0 )
	{
		EV_SET(&ke, fd, EVFILE_READ, EV_ADD, 0, 0, NULL);
		kevent(m_kqfd , &ke, 1 , NULL, 0, NULL);
	}

	if((socket_event & SOCKET_WRITE) != 0)
	{
		EV_SET(&ke, fd, EVFILE_WRITE , EV_ADD, 0,0,NULL);
		kevent(m_kqfd, &ke, 1, NULL, 0, NULL);
	}
}
...
void CEventDispatch::RemoveEvent(SOCKET fd, uint8_t socket_event)
{
	...
	if((socket_event & SOCKET_READ) != 0)
	{
		EV_SET(&ke, fd, EVFILE_READ, EV_DELETE, 0, 0, NULL);
		kevent(m_kqfd, &ke, 1, NULL, 0, NULL);
	}

	if((socket_event & SOCKET_WRITE) != 0)
	{
		EV_SET(&ke, fd, EVFILE_WRITE, EV_DELETE, 0, 0, NULL);
		kevent(m_kqfd, &ke, 1, NULL , 0, NULL);
	}
}
